...
def qrcodeview():
    ...
    for i in range(0,len(qrcode_detail)):
        ...
        qrcode_img = qrcode.make(data="company:ACEN,QRcodeID:{},Description:{},LocationID:{}".format(qrcode_detail[i][0],qrcode_detail[i][1],qrcode_detail[i][2])) 
        qrcode_img.save(r"C:\Users\Dell\Desktop\Major Project\qrcodes\{}.png".format(qrcode_detail[i][0]))
        img = read_file(r"C:\Users\Dell\Desktop\Major Project\qrcodes\{}.png".format(qrcode_detail[i][0]))
...
def extractor(decoded_string):
    ...
    flag= (data[0][1])
    if flag.lower() != 'acen':
        msg = "NOT OWNED BY ACEN, things printed on qrcode are :" + decoded_string 
        return msg,False
    else:
