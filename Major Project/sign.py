...
def client_register():
    ...
    if request.method=='POST':
        ...
        cursor = mysql.cursor()
        cursor.execute('''INSERT INTO ClientDB(clientID,location,ContactName,mobileNumber,EmailAddress,pinCode,city,state,password) VALUES(?,?,?,?,?,?,?,?,?)''' ,clientID,location,ContactName,ContactNumber,Contactemail,pincode,city,state,password)
        mysql.commit()
        cursor.close()
        msg='registered successfully '
